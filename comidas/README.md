...
Al abrir, Registrar cae en la comida que ya tiene algo registrado hoy (la de la hora si tiene; si no, la última registrada); si no hay nada, elige por la hora. En la tira de días y en el selector de comidas, los días y comidas con registros llevan un puntito lavanda.

Cuatro pestañas abajo: **Registrar** (abre siempre en el día de hoy; se puede cambiar a ayer u otro día), **Semana**, **Mes** y **Planificar**. El ⚙️ de arriba a la derecha abre un menú con Exportar Excel (con la fecha de tu última copia), Importar Excel, un interruptor de Sonido al sumar y Ajustes.
...
- **Sonido "Faaa"** al sumar (estilo videos de bromas): sintetizado en la app con Web Audio, sin archivos; más largo en la comida completa. En ⚙️ Ajustes se apaga, se prueba o se reemplaza por un audio tuyo (menos de 1 MB, queda guardado en el iPhone). Suena como un video (con el volumen multimedia), también con el iPhone en silencio.
- **Contar en** (⚙️ Ajustes): por defecto todo se muestra en comidas libres con tercios (⅓, ⅔, 1⅔). Con "Porciones enteras", cada parte vale 1 y la completa 3 (cupo de 2 comidas libres = 6 porciones): la app muestra solo números enteros, sin fracciones. Los datos y el Excel no cambian.
